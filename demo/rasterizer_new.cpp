...
const int coeff_precision_base = 22; //11 new 20 old
//Fixedpoint base for NDC coordinates (we convert screen-space coords to NDCs later)
const int ndc_precision_base = 22; //26 new 20 old
...
void BlitTilesCombined()
{
	...
	unsigned int* colorbuffer = (unsigned int*)wc_colorbuffer->Lock();
	...
	for(int y = 0; y < height; y+= q) {
		for(int x = 0; x < width; x+= q) {
			...
			const int tileX = x >> Q;
			const int tileY = y >> Q;
			size_t tileListIndex = tileX + tileY*numTilesX;
			//assert(tileListIndex < wc_tileList.size());
			//assert(tileListIndex < wc_tileListFilled.size());
			if(!(tileListIndex < wc_tileList.size() && tileListIndex < wc_tileListFilled.size()))
			   continue;
			TileSet& tileSet = wc_tileList[tileListIndex];
			TileSet& tileSetFilled = wc_tileListFilled[tileListIndex];
				...
				for(int iy = 0; iy < q; ++iy) {
					...
					for(int ix = 0; ix < q; ++ix) {
						...
						if(z < depthbuffer[zIndex]) {
							...
							if(w) w = (1LL<<(coeff_precision_base * 2)) / w;
							//if(w) w = reci20(w);
	...
	for(int i=0; i<wc_vertices->size(); i+=3) {
		...
		for(int y = miny; y < maxy; y += q) {
			for(int x = minx; x < maxx; x += q) {
				...
				/* Convert from NDC base to coeff base by shifting
				   down the difference */
				...
				//Compute w for the corners of the tile. Result is in
				//coeff base
				int bwi0 = (((long long)Aw*bwx0 + (long long)Bw*bwy0 + ((long long)Cw << coeff_precision_base)) >> coeff_precision_base); //top left
				int bwi1 = (((long long)Aw*bwx0 + (long long)Bw*bwy1 + ((long long)Cw << coeff_precision_base)) >> coeff_precision_base); //bottom left
				int bwi2 = (((long long)Aw*bwx1 + (long long)Bw*bwy0 + ((long long)Cw << coeff_precision_base)) >> coeff_precision_base); //top right
				int bwi3 = (((long long)Aw*bwx1 + (long long)Bw*bwy1 + ((long long)Cw << coeff_precision_base)) >> coeff_precision_base); //bottom right

				//Compute z for the corners of the tile. Result is in
				//depth base
				int bz0 = (((long long)Az*bzx0 + (long long)Bz*bzy0 + ((long long)Cz << depth_precision_base)) >> depth_precision_base); //top left
				int bz1 = (((long long)Az*bzx0 + (long long)Bz*bzy1 + ((long long)Cz << depth_precision_base)) >> depth_precision_base); //bottom left
				int bz2 = (((long long)Az*bzx1 + (long long)Bz*bzy0 + ((long long)Cz << depth_precision_base)) >> depth_precision_base); //top right
				int bz3 = (((long long)Az*bzx1 + (long long)Bz*bzy1 + ((long long)Cz << depth_precision_base)) >> depth_precision_base); //bottom right

				//Compute u for the corners of the tile
				int bu0 = (((long long)Au*bwx0 + (long long)Bu*bwy0 + ((long long)Cu << coeff_precision_base)) >> coeff_precision_base); //top left
				int bu1 = (((long long)Au*bwx0 + (long long)Bu*bwy1 + ((long long)Cu << coeff_precision_base)) >> coeff_precision_base); //bottom left
				int bu2 = (((long long)Au*bwx1 + (long long)Bu*bwy0 + ((long long)Cu << coeff_precision_base)) >> coeff_precision_base); //top right
				int bu3 = (((long long)Au*bwx1 + (long long)Bu*bwy1 + ((long long)Cu << coeff_precision_base)) >> coeff_precision_base); //bottom right

				//Compute v for the corners of the tile
				int bv0 = (((long long)Av*bwx0 + (long long)Bv*bwy0 + ((long long)Cv << coeff_precision_base)) >> coeff_precision_base); //top left
				int bv1 = (((long long)Av*bwx0 + (long long)Bv*bwy1 + ((long long)Cv << coeff_precision_base)) >> coeff_precision_base); //bottom left
				int bv2 = (((long long)Av*bwx1 + (long long)Bv*bwy0 + ((long long)Cv << coeff_precision_base)) >> coeff_precision_base); //top right
				int bv3 = (((long long)Av*bwx1 + (long long)Bv*bwy1 + ((long long)Cv << coeff_precision_base)) >> coeff_precision_base); //bottom right

				int bw0, bw1, bw2, bw3;
				bw0 = bw1 = bw2 = bw3 = 0;

				//Uncomment to interpolate 'w' instead of '1/w' for a quad
				/*
				if(bwi0) bw0 = (1LL<<(coeff_precision_base * 2)) / bwi0;
				if(bwi1) bw1 = (1LL<<(coeff_precision_base * 2)) / bwi1;
				if(bwi2) bw2 = (1LL<<(coeff_precision_base * 2)) / bwi2;
				if(bwi3) bw3 = (1LL<<(coeff_precision_base * 2)) / bwi3;
				*/
